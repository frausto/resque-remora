...
Resque Remora is a plugin for the Resque queueing system (http://github.com/defunkt/resque) that allows you to attach a hash with whatever information you want to resque jobs when they are enqueued (much like how a remora attached to a shark... perhaps), and then to get that information back and process it whenever the job is taken off the queue. The hash is attached directly to the job so it can be viewed in Redis with the jobs other information. 
...
For Example:
  ...
  class Job
    ...
    def process_remora(queue, information)
      #do whatever I want with this information
    end
